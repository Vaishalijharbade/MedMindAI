...
const profile = user.emergencyProfile
  ...
  return (
    <div className="min-h-screen py-8 px-4" style={{ background: 'linear-gradient(135deg, #060b14, #0a1020)' }}>
      <div className="max-w-sm mx-auto">
        ...
        <div className="text-center mb-6">
          ...
          <p className="text-slate-400 text-sm">Medical Information Card</p>
        </div>

        {/* Patient Name & Blood Group */}
        <div className="glass-card p-5 mb-4 text-center" style={{ borderColor: 'rgba(239,68,68,0.3)' }}>
          <p className="text-lg font-bold text-white">{name}</p>
          <div className="inline-flex items-center gap-2 mt-2 px-4 py-2 rounded-xl"
               style={{ background: 'linear-gradient(135deg, #ef4444, #f43f5e)', boxShadow: '0 0 20px rgba(239,68,68,0.4)' }}>
            <Droplets size={18} className="text-white" />
            <span className="text-2xl font-black text-white">{profile.bloodGroup}</span>
          </div>
        ...
        {profile.allergies?.length > 0 && (
          <div className="glass-card p-4 mb-4" style={{ borderColor: 'rgba(239,68,68,0.2)' }}>
            <h2 className="text-xs font-bold text-rose-400 uppercase tracking-wider mb-3 flex items-center gap-2">
              <AlertTriangle size={13} /> ⚠️ ALLERGIES
            </h2>
            <div className="flex flex-wrap gap-2">
              {profile.allergies.map((a: string, i: number) => (
                <span key={i} className="px-2.5 py-1 rounded-lg text-sm font-medium text-rose-200 bg-rose-500/20 border border-rose-500/30">
                  {a}
                </span>
              ))}
            </div>
        ...
        {profile.chronicDiseases?.length > 0 && (
          <div className="glass-card p-4 mb-4">
            <h2 className="text-xs font-bold text-amber-400 uppercase tracking-wider mb-3">🏥 Medical Conditions</h2>
            <div className="flex flex-wrap gap-2">
              {profile.chronicDiseases.map((d: string, i: number) => (
                <span key={i} className="px-2.5 py-1 rounded-lg text-sm text-amber-200 bg-amber-500/20 border border-amber-500/30">{d}</span>
              ))}
            </div>
          </div>
        )}
        ...
        {/* Emergency Contacts */}
        {contacts.length > 0 && (
          <div className="glass-card p-4 mb-4" style={{ borderColor: 'rgba(16,185,129,0.2)' }}>
        ...
        {(profile.doctorName || profile.doctorPhone) && (
          <div className="glass-card p-4 mb-4">
            <h2 className="text-xs font-bold text-pink-400 uppercase tracking-wider mb-3 flex items-center gap-2">
              <Heart size={12} /> Treating Doctor
            </h2>
            <div className="flex items-center justify-between">
              <p className="text-sm text-white">{profile.doctorName}</p>
              {profile.doctorPhone && (
                <a href={`tel:${profile.doctorPhone}`}
                  className="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm text-white"
                  style={{ background: 'rgba(236,72,153,0.2)', border: '1px solid rgba(236,72,153,0.3)' }}>
                  <Phone size={12} /> {profile.doctorPhone}
                </a>
              )}
            </div>
          </div>
        ...
        {profile.notes && (
          <div className="glass-card p-4 mb-6">
            <h2 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-2">Additional Notes</h2>
        ...
        <div className="text-center">
          <div className="flex items-center justify-center gap-2 mb-2">
            ...
            <span className="text-xs font-semibold gradient-text">MedMind AI</span>
          </div>
          <p className="text-xs text-slate-600">Secure Medical ID · Generated by MedMind AI</p>
        </div>
      </div>
    </div>
  )
